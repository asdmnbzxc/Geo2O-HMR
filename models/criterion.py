...
def matchability_aware_loss(inputs, targets, match_quality, positive_idx,
                            valid_mask=None, gamma: float = 1.5, eps: float = 1e-6):
    """DEIM-style MAL adapted to Geo2O-HMR's sigmoid confidence head."""
    prob = inputs.clamp(min=eps, max=1.0 - eps)
    loss = -(prob ** gamma) * torch.log(1.0 - prob)

    if match_quality.numel() > 0:
        quality = match_quality.to(device=prob.device, dtype=prob.dtype).clamp(0.0, 1.0).pow(gamma).view(-1, 1)
        pos_prob = prob[positive_idx]
        pos_loss = -(quality * torch.log(pos_prob) + (1.0 - quality) * torch.log(1.0 - pos_prob))
        loss[positive_idx] = pos_loss
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, matcher, weight_dict, losses = ['confs','boxes', 'poses','betas', 'j3ds','j2ds', 'depths', 'kid_offsets'], 
                ...
                conf_loss_type='focal', mal_gamma=1.5):
    ...
    def loss_confs(self, loss, outputs, targets, indices, num_instances, is_dn=False, **kwargs):
        ...
        with torch.no_grad():
            ...
            detect_all = torch.tensor([t['detect_all_people'] for t in targets],
                                      dtype=torch.bool, device=pred_confs.device)
            valid_batch_idx = torch.where(detect_all)[0]
            detection_valid_mask[valid_batch_idx] = True

        
        losses = {}
        if self.conf_loss_type == 'mal':
            match_quality = self._matched_box_ious(outputs, targets, indices)
            valid_mask = None if is_dn else detection_valid_mask
            losses[loss] = matchability_aware_loss(
                pred_confs, labels, match_quality, idx,
                valid_mask=valid_mask, gamma=self.mal_gamma) / num_instances
        else:
            if is_dn:
    ...
    def _matched_box_ious(self, outputs, targets, indices):
        match_count = sum(len(src) for src, _ in indices)
        if match_count == 0:
            return torch.empty(0, device=outputs['pred_boxes'].device)
